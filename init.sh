...
install_Python3_env_and_create_auto_pppoe_to_run() {
    check_configure_dns() {
    ...
    pip3 install psutil -i https://mirrors.aliyun.com/pypi/simple/ --trusted-host pypi.douban.com &> /dev/null
...
create_systemd_service() {
    ...
    echo "$(date "+%Y-%m-%d %H:%M:%S") 开始创建auto_downloader.py脚本并写进系统服务运行..."
    ...
    systemctl start auto_downloader.service &> /dev/null
    echo "$(date "+%Y-%m-%d %H:%M:%S") auto_downloader.py脚本已创建并写进系统服务并设置成开机自启"
}
check_log() {
    log_file="/opt/auto_pppoe/log/auto_pppoe.log"
...
# 写入machineTag到系统内
mkdir -p /opt/auto_pppoe/info && echo "@@MACHINETAG@@" > /opt/auto_pppoe/info/machineTag.info
echo "$(date "+%Y-%m-%d %H:%M:%S") machineTag已写入系统内"

# 下载auto_pppoe脚本程序
curl -o /opt/auto_pppoe/Auto_PPPoE_v1.0.tar.gz -L https://gitee.com/yuanzichaopu/auto_pppoe/releases/download/auto_pppoe_v1.0/Auto_PPPoE_v1.0.tar.gz
...
# 启动服务并检查日志
touch /opt/auto_pppoe/log/auto_pppoe.log
create_auto_pppoe_as_systemd_service
tail -f /opt/auto_pppoe/log/auto_pppoe.log &
TAIL_PID=$!
check_log
kill $TAIL_PID